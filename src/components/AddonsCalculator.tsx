...
import { PhotoPackage, AddOnItem } from '../types';
import { PHOTO_PACKAGES, ADD_ON_SERVICES } from '../data/mockData';
import { formatRupiah } from '../utils/formatters';
import { Calculator, Check, Plus, ArrowRight, Sparkles } from 'lucide-react';
...
export const AddonsCalculator: React.FC<AddonsCalculatorProps> = ({
  ...
  return (
    <section id="kalkulator-simulasi" className="py-16 bg-[#0A0A0A] text-[#E0E0E0] border-b border-white/10">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto mb-12">
          ...
          <p className="mt-3 text-gray-400 text-sm sm:text-base leading-relaxed">
            Sesuaikan kebutuhan sesi foto Anda dengan memilih paket utama dan layanan ekstra (add-ons) sesuai konsep impian Anda.
          </p>
        </div>
                ...
                {packages.map((pkg) => {
                  ...
                  return (
                    <button
                      ...
                      className={`p-3.5 text-left border transition-all flex flex-col justify-between cursor-pointer ${
                        isSelected
                    ...
                    >
                      <div className="flex items-start justify-between">
                        <span className="font-semibold text-xs text-white line-clamp-1">{pkg.name}</span>
                        {isSelected && <Check className="w-4 h-4 text-[#D4AF37] shrink-0 ml-1" />}
                      </div>
                      <div className="mt-3 flex items-baseline justify-between pt-2 border-t border-white/5">
                        <span className="text-[10px] text-gray-500 font-mono uppercase">{pkg.duration}</span>
